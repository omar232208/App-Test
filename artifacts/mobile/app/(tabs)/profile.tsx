...
function PlanBadge({ plan }: { plan: string }) {
  ...
  const config: Record<string, { color: string; label: string }> = {
    free: { color: colors.mutedForeground, label: 'Free' },
  ...
  return (
    <View style={[styles.planBadge, { backgroundColor: c.color + '18', borderColor: c.color + '44' }]}>
      <Text style={{ color: c.color, fontSize: 11, fontFamily: 'Inter_700Bold', letterSpacing: 0.5 }}>{c.label}</Text>
    </View>
  );
}
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.modal, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
          ...
          <Pressable onPress={() => { if (name.trim()) { onSave(name.trim()); onClose(); } }}><Text style={[styles.modalSave, { color: colors.primary }]}>Save</Text></Pressable>
        </View>
        <View style={{ padding: 20 }}>
          ...
          <View style={[styles.fieldWrap, { borderColor: colors.primary, backgroundColor: colors.card }]}>
            ...
            <TextInput style={[styles.fieldInput, { color: colors.foreground }]} value={name} onChangeText={setName} placeholder="Your name" placeholderTextColor={colors.mutedForeground} autoFocus returnKeyType="done" />
          </View>
        </View>
      </View>
    </Modal>
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.modal, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
          ...
          <Pressable onPress={doChange}><Text style={[styles.modalSave, { color: colors.primary }]}>Save</Text></Pressable>
        </View>
          ...
          {[
            ...
            <View key={i}>
              ...
              <View style={[styles.fieldWrap, { borderColor: colors.border, backgroundColor: colors.card }]}>
                ...
                <TextInput style={[styles.fieldInput, { color: colors.foreground }]} value={f.value} onChangeText={f.set} placeholder="••••••••" placeholderTextColor={colors.mutedForeground} secureTextEntry returnKeyType="next" />
              </View>
            </View>
          ))}
        </View>
...
function SessionsModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.from('sessions').select('*').eq('user_id', user.id).order('last_active', { ascending: false }).then(({ data }) => setSessions(data ?? []));
  }, [visible, user]);
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.modal, { backgroundColor: colors.background }]}>
        <View style={[styles.modalHandle, { backgroundColor: colors.border }]} />
        <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
          <Pressable onPress={onClose}><Text style={[styles.modalCancel, { color: colors.mutedForeground }]}>Done</Text></Pressable>
          <Text style={[styles.modalTitle, { color: colors.foreground }]}>Active Sessions</Text>
          <View style={{ width: 40 }} />
        </View>
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          {sessions.length === 0 ? (
            <Text style={{ color: colors.mutedForeground, textAlign: 'center', marginTop: 40 }}>No active sessions</Text>
          ) : sessions.map((s, i) => (
            <View key={s.id} style={[styles.sessionCard, { backgroundColor: colors.card, borderColor: colors.border, marginBottom: 10 }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <Feather name={s.device_type === 'mobile' ? 'smartphone' : 'monitor'} size={18} color={colors.primary} />
                <View style={{ flex: 1 }}>
                  <Text style={[styles.themeOptLabel, { color: colors.foreground }]}>{s.device || 'Unknown device'}</Text>
                  <Text style={[styles.themeOptDesc, { color: colors.mutedForeground }]}>{s.location || 'Unknown location'} · {s.ip}</Text>
                </View>
                <View style={[styles.statusDotS, { backgroundColor: '#22C55E' }]} />
              </View>
              <Text style={[styles.themeOptDesc, { color: colors.mutedForeground }]}>Last active: {new Date(s.last_active).toLocaleString()}</Text>
            </View>
          ))}
        </ScrollView>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: botPad + 110 }}>
        <LinearGradient colors={['#080818', '#0F0F28']} style={[styles.hero, { paddingTop: topPad + 20 }]}>
          ...
          <Animated.View entering={FadeInDown.duration(400)} style={styles.avatarWrap}>
            ...
            <Pressable onPress={() => setEditModal(true)} style={[styles.avatarEdit, { backgroundColor: colors.primary, borderColor: '#080818' }]}>
        ...
        <View style={styles.body}>
          ...
          <Section title="PLAN">
            <SettingRow icon="award"     label="Current Plan"  value={plan === 'free' ? 'Free' : plan.charAt(0).toUpperCase() + plan.slice(1)} iconColor="#F59E0B" delay={130} />
            <SettingRow icon="arrow-up"  label="Upgrade to Pro"
              onPress={() => Alert.alert('Upgrade', 'Coming soon — Unlock AI agents, advanced analytics, and more.')}
              iconColor="#6366F1" delay={150}
            />
            <SettingRow icon="shopping-bag" label="Manage Subscription" value="Coming soon" delay={170} />
          </Section>

          <Section title="APPEARANCE">
            <SettingRow icon="moon"   label="Theme"       value={themeLabel} onPress={() => setThemeModal(true)} iconColor="#818CF8" delay={200} />
            <SettingRow icon="layout" label="Compact Mode" toggle toggleValue={compact} onToggle={setCompact} iconColor="#8B5CF6" delay={230} />
          </Section>

          <Section title="NOTIFICATIONS">
            <SettingRow icon="bell"   label="Push Notifications" toggle toggleValue={notifications} onToggle={setNotifications} iconColor="#EC4899" delay={270} />
            <SettingRow icon="mail"   label="Email Digest"       toggle toggleValue={emailDigest} onToggle={setEmailDigest} iconColor="#6366F1" delay={300} />
          </Section>

          <Section title="SECURITY CENTER">
            <SettingRow icon="lock"        label="Change Password"   onPress={() => setPasswordModal(true)} iconColor="#EF4444" delay={340} />
            <SettingRow icon="shield"      label="Two-Factor Auth"   value="Coming soon" iconColor="#F59E0B" delay={370} />
            <SettingRow icon="smartphone"  label="Active Sessions"   onPress={() => setSessionsModal(true)} iconColor="#3B82F6" delay={400} />
            <SettingRow icon="key"         label="API Keys"          value="Coming soon" iconColor="#14B8A6" delay={430} />
            <SettingRow icon="log-in"      label="Login History"     value="Coming soon" iconColor="#8B5CF6" delay={460} />
          </Section>

          <Section title="DATA & STORAGE">
            <SettingRow icon="download"  label="Export Data"  onPress={() => showExport({ projects, notes, folders, bookmarks })} iconColor="#14B8A6" delay={500} />
            <SettingRow icon="database"  label="Storage Used"  value={`~${storageKB} KB`} delay={530} />
            <SettingRow icon="activity"  label="Activity Log"  value={`${unreadCount} events`} delay={560} />
            <SettingRow icon="refresh-cw" label="Sync Status"  value="Up to date" delay={590} />
          </Section>
          ...
          <Text style={[styles.footer, { color: colors.mutedForeground }]}>DevOS v2.0.0 · Made with ❤️ for developers</Text>
        </View>
      </ScrollView>

      <ThemeModal    visible={themeModal}    current={theme} onClose={() => setThemeModal(false)} onSelect={setTheme} />
      <EditProfileModal visible={editModal} current={user?.name ?? ''} onClose={() => setEditModal(false)} onSave={(name) => updateUser({ name })} />
      <ChangePasswordModal visible={passwordModal} onClose={() => setPasswordModal(false)} />
      <SessionsModal visible={sessionsModal} onClose={() => setSessionsModal(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  planBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8, borderWidth: 1, marginTop: 4 },
  sessionCard: { borderRadius: 14, borderWidth: 1, padding: 14 },
  statusDotS: { width: 8, height: 8, borderRadius: 4 },
});
